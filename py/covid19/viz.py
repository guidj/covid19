...
def create_viz_growth_simulation(charts_path: str):
...
def create_viz_line_chart_with_ruler_as_selector(df: pd.DataFrame):
    # Create a selection that chooses the nearest point & selects based on x-value
    nearest_selection = alt.selection(
        type="single", nearest=True, on="mouseover", fields=["x"], empty="none"
    )


def create_viz_world_confirmed_and_rate(charts_path: str, dfs: Dict[str, pd.DataFrame]):
    ...
    alt.vconcat(
        add_ruler_as_selector_on_single_line_chart(
            world_agg_confirmed_chart, df_world_agg_confirmed, "date", "count"
        ),
        add_ruler_as_selector_on_single_line_chart(
            world_agg_growth_rate_chart, df_world_agg_growth_rate, "date", "growth_rate"
        ),
    ).save(viz_path, embed_options={"renderer": "svg"})


def add_ruler_as_selector_on_single_line_chart(chart, df, x_field, y_field):
    # Create a selection that chooses the nearest point & selects based on x-value
